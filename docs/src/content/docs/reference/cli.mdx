...
Scaffolds a new Calimero application from the KV Store reference app in the
...
1. Validates the project name as an npm package name; refuses a non-empty
   existing directory.
2. Sparse-checks-out `apps/kv-store` from `calimero-network/apps` into a temp
   directory. Cone mode also brings the repo-root files, which the next step
   needs.
3. Copies the app into your project, excluding `.git`, `.github`, `.gitignore`,
...
5. Writes a project root (`package.json`, `pnpm-workspace.yaml`) and a README
   describing the standalone commands.
...
:::caution[A scaffolded project does not track the template]
Versions are pinned at scaffold time. The project upgrades on its own schedule
from then on — it is a starting point, not a live link to `apps`.
:::
