...
- **abi-codegen:** sync the bundled `wasm-abi-v1.schema.json` with calimero core
  so all current-core ABIs parse again. Widen the `CrdtType` enum from 6 to all
  11 CRDT collection types (`sorted_map`, `authored_map`, `sorted_set`,
  `authored_vector`, `shared_storage` were missing), relax `BytesType.encoding`
  to a string, and add the `state_version`, `migrations`, method `intent`, and
  `xcall_callable` fields to the schema and `src/model.ts`. The published
  `0.1.1`/`1.0.3` schemas rejected every current-core ABI; the real corpus now
  parses 20/20.
...
- Add cross-repo guards that run on every PR: a corpus parse test over a
  vendored snapshot of every core ABI + mero-drive, a schema drift check against
  core's `wasm-abi.schema.json`, and an enum-completeness mirror of core's Rust
  `MethodIntent`/`CrdtCollectionType`. Re-vendor with `pnpm run sync:corpus`.

- Re-vendor the core reference files at `0.11.0-rc.25`. The bundled schema was
  already in sync, so nothing shipped changes; the stale snapshot was failing
  the `Schema parity vs released core` job. Core's conformance app grew
  `tuple_pair`/`list_tuples` (tuples became a positional ABI type) and dropped
  `ConformanceError`, so the hardcoded fixture counts move to 40 methods / 17
  types and the variant assertions move to `Status`. Also drop the
  `tuple` entry from the schema-drift deviation list: core emits tuples now, so
  the branch is no longer abi-codegen-only.

> Note: releases are managed by semantic-release (see `RELEASING.md`); the
> entries below were generated by earlier release tooling.
