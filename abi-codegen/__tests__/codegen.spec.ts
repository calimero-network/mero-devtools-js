...
describe("Codegen", () => {
  ...
  describe("client.ts generation", () => {
    it("should generate client.ts with correct structure", () => {
      ...
      expect(clientContent).toContain(
        "const result = await this._mero.rpc.execute({ contextId: this._contextId, method: 'opt_u32', argsJson: params, executorPublicKey: this._executorPublicKey });",
      );
      expect(clientContent).toContain(
        "async makePerson(params: { p: Person }): Promise<Person> {",
      );
      expect(clientContent).toContain(
        "const result = await this._mero.rpc.execute({ contextId: this._contextId, method: 'make_person', argsJson: convertCalimeroBytesForWasm(params), executorPublicKey: this._executorPublicKey });",
      );
    ...
    it("should handle single-parameter methods correctly", () => {
      ...
      expect(clientContent).toContain(
        "const result = await this._mero.rpc.execute({ contextId: this._contextId, method: 'roundtrip_id', argsJson: convertCalimeroBytesForWasm(params), executorPublicKey: this._executorPublicKey });",
      );
      expect(clientContent).toContain(
        "async optU32(params: { x: number | null }): Promise<number | null> {",
      );
    ...
    it("should handle methods with errors correctly", () => {
      ...
      // may_fail declares no `errors` in the ABI, so it is a plain method:
      // rpc.execute throws on failure and the body just returns the result.
      expect(clientContent).toContain("return result as number;");
    });

    it("should map typed errors from RpcError for methods that declare errors", () => {
      const abi = {
        ...
        methods: [
          {
            name: "doThing",
            params: [{ name: "flag", type: { kind: "bool" } }],
            returns: { kind: "string" },
            errors: [{ code: "BadInput" }],
          },
        ],
        events: [],
      };
      ...
      // RpcError is imported only because a method declares typed errors.
      expect(clientContent).toContain("  RpcError,");
      // The call is wrapped so the structured payload on RpcError.data can be
      // surfaced as the generated error union.
      expect(clientContent).toContain("    try {");
      expect(clientContent).toContain(
        "      if (error instanceof RpcError && error.data && typeof error.data === 'object') {",
      );
      expect(clientContent).toContain(
        "        throw error.data as doThingError;",
      );
    });

    it("should not import RpcError when no method declares errors", () => {
      // The conformance fixture has no `errors` on any method.
      const clientContent = generateClient(manifest);
      expect(clientContent).not.toContain("  RpcError,");
      expect(clientContent).not.toContain("throw error.data");
    });
