...
`;
    return imports + errorTypes.join("\n") + "\n\n" + paramTypes.join("\n") + "\n\n" + returnTypes.join("\n") + "\n" + errorInterface;
  }
  /**
...
var AbiCodegen = class {
  ...
  generateFiles(abi, outputDir) {
    ...
    const typesPath = (0, import_path.join)(outputDir, "types.ts");
    (0, import_fs.writeFileSync)(typesPath, typesContent, "utf-8");
    const clientContent = this.clientGenerator.generateClientFile(abi);
    const clientPath = (0, import_path.join)(outputDir, "client.ts");
    (0, import_fs.writeFileSync)(clientPath, clientContent, "utf-8");
...
program2.name("calimero-abi-codegen").description("Generate TypeScript types and client from Calimero ABI schema").version("0.1.0").requiredOption("-i, --input <file>", "Input ABI JSON file").requiredOption("-o, --output <dir>", "Output directory for generated files").action(async (options) => {
  try {
