 ...
 */
export function generateTypes(manifest) {
    const lines = [];
    // Add file banner
 ...
 */
function generateTypeRef(typeRef, manifest, forUserApi = false, forVariantParam = false) {
    if ('$ref' in typeRef) {
 ...
 */
function generateMethodErrorTypes(method) {
    const lines = [];
    ...
    const errorVariants = method.errors.map((error) => {
        if (error.payload) {
            const payloadType = generateTypeRef(error.payload, method);
...
function generateEventPayloadType(event) {
    ...
    if (event.payload &&
        ...
        const payloadType = generateTypeRef(event.payload, event);
 ...
 */
function generateAbiEventUnion(events) {
    const lines = [];
