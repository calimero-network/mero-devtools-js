 ...
 */
export function parseAbiManifest(json) {
    ...
    if (manifest.schema_version !== 'wasm-abi/1') {
        throw new Error(`Invalid schema version: expected "wasm-abi/1", got "${manifest.schema_version}"`);
    }
 ...
 */
export function loadAbiManifestFromFile(path) {
    try {
...
function validateInvariants(manifest) {
    ...
    const checkTypeRefs = (typeRef, context) => {
    ...
    for (const method of manifest.methods) {
        ...
        if (method.errors) {
            for (const error of method.errors) {
                if (error.payload) {
                    checkTypeRefs(error.payload, `method.${method.name}.error.${error.code}`);
                }
            }
        }
    }
    ...
    for (const [typeName, typeDef] of Object.entries(manifest.types)) {
        if (typeDef.kind === 'record') {
            ...
            for (const variant of typeDef.variants) {
                if (variant.payload) {
                    checkTypeRefs(variant.payload, `type.${typeName}.variant.${variant.name}`);
                }
            }
        }
    }
    // Check that map keys are strings (schema should enforce this, but double-check)
    const validateMapKeys = (typeRef, context) => {
        if (isTypeRef(typeRef)) {
